...
const getHomeLayout = async (req, res) => {
    try {
        ...
        // 3. Featured Salons & Freelancers (Actual Data Only)
    ...
    // 4. Gender-specific Categories (with images)
    const [allCategories, femaleCategoriesRaw, maleCategoriesRaw] = await Promise.all([
    ...
    allCategories.forEach(c => {
      categoryMap[c.name] = c.image ? getCloudinaryUrl(c.image) : null;
    });

    const enrichCategories = (cats) => cats.map(c => ({
      name: c.category,
      image: categoryMap[c.category] || null
    }));
      ...
      sections: [
        {
          ...
          gender: "Female",
          categories: enrichCategories(femaleCategoriesRaw)
        },
        {
          title: "Salon for Men",
          gender: "Male",
          categories: enrichCategories(maleCategoriesRaw)
        }
      ]
    });
  } catch (error) {
